...
echo  ""
echo -e "Если в списке ниже вы видите ${RED}su${NC}, значит все впорядке, рут права предоставлены, приложение установлено."
echo -e "Если в списке ниже отсутствует ${RED}su${NC}, необходимо выполните установку приложения ${RED}Super SU${NC} и обновите бинарный файл."
echo ""
adb shell "su -c ls /system/xbin"
echo -e "${WHITE}...${NC}\n"
sh $0
}
###########################
...
echo "Удаление приложений Painter, Weather, Map, Reader, EBook, Sogou"
...
adb shell su -c rm -r /system/app/Painter >/dev/null 2>&1
adb shell su -c rm -r /system/app/Weather >/dev/null 2>&1
#adb shell su -c rm -r /system/app/Map >/dev/null 2>&1
...
ACT_2="Установка SuperSU (ver.2.79)"
ACT_3="Удаление китайского софта"
ACT_4="Заморозка софта"
ACT_5="Перевод прошивки"
ACT_6="Проверка ID устройства"
ACT_7="Подмена ID устройства под международную прошивку"
ACT_8="Подмена ID устройства под китайскую прошивку"
ACT_9="Подмена ID устройства под бета прошивку"
ACT_10="Установка пользовательских приложений (папка files/user_app)"
ACT_11="Удаление пользовательских приложений с телефона"
ACT_12="Выход"

PS3="${BLU1}Ваш выбор пользователь: ${NC1}"

select CHOICES in "${ACT_1}" "${ACT_2}" "${ACT_3}" "${ACT_4}" "${ACT_5}" "${ACT_6}" "${ACT_7}" "${ACT_8}" "${ACT_9}" "${ACT_10}" "${ACT_11}" "${ACT_12}"; do
    case ${CHOICES} in
        "${ACT_1}")
            CHK_SU
            ;;
